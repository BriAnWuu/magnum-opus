...
function SocketBroadcast({ userId }) {
    ...
    const [numberOfMessage, setNumberOfMessage] = useState(2);

    const onReceive = (auction) => {
        const message = `There is a new bid on Lot ${auction.id}`;
    ...
    return (
        ...
        >
            ...
            <Badge 
                ...
                badgeContent={ numberOfMessage } 
                max={99}
                color="error"
                aria-label= {`${numberOfMessage} notifications unread`}
            >
                <div 
